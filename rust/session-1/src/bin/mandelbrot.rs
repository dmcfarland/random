...
fn main() {
    ...
    if args.len() != 5 {
        eprintln!(
            "Usage: {} <file> <pixels> <update-left> <update-righ>",
            args[0]
        );
        eprint!(
            "Example: {} mandel-a.png 1000x750 -1.20,0.35 -1,0.20",
            args[0]
        );
        std::process::exit(1);
    }
...
fn excape_time(c: Complex<f64>, limit: usize) -> Option<usize> {
...
fn render(
    ...
    for row in 0..bounds.1 {
        for col in 0..bounds.0 {
            ...
            pixels[row * bounds.0 + col] = match excape_time(point, 255) {
                None => 0,
